...
class Coineal():
    def __init__(self, base_url='https://exchange-open-api.coineal.com'):
    ...
    def signed_request(self, method, api_url, **payload):
        ...
        try:
            r = requests.request(
                method, full_url, headers=headers, json=payload)
    ...
    def get_market_depth(self, type, symbol):
        """get market depth"""
        return self.public_request('GET', '/open/api/market_dept?symbol={symbol}&type={type}'.format(type=type, symbol=symbol))
